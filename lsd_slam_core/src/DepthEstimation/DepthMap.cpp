...
int DepthMap::debugPlotDepthMap()
{
	...
	for(int y=0;y<height;y++)
		for(int x=0;x<width;x++)
		{
			...
			//std::cout << currentDepthMap[idx]-> idepth << std::endl;
			if (currentDepthMap[idx].idepth > max)
				max = currentDepthMap[idx].idepth_smoothed;
			if (currentDepthMap[idx].idepth < min)
				min = currentDepthMap[idx].idepth_smoothed;

		}
	std::cout << "MAX VALUE = " << max << "	MIN VALUE= " <<  min << std::endl;

    //cv::imwrite("/home/hamid/slam/1.jpg",debugImageDepth);
	return 1;
}
